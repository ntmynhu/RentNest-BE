...
class UserController {
  ...
  warn = async (req: Request, res: Response) => {
    ...
    const warning = await userService.warnUser(req.user!.id, parseInt(req.params.id), reason)
    return new SuccessResponse({ message: 'Warning issued', metaData: warning }).send(res)
  }

  ban = async (req: Request, res: Response) => {
    const { reason } = req.body
    const user = await userService.banUser(req.user!.id, parseInt(req.params.id), reason)
